...
def get_curso(curso_id: str) -> dict | None:
    return next((c for c in CURSOS if c["id"] == curso_id), None)


def get_disciplinas(curso_id: str, ano_letivo: str) -> list[str]:
    curso = get_curso(curso_id)
    if not curso:
        return []
    for ano in curso["anos"]:
        if ano["ano"] == ano_letivo:
            return ano["disciplinas"]
    return []
